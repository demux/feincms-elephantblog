...
def entry_list_lookup_related(entry_qs):
    ...
    if hasattr(Entry, 'markupmirrorcontent_set'):
        for content in Entry.markupmirrorcontent_set.related.model.objects.filter(
                parent__in=entry_dict.keys()).reverse():
            entry_dict[content.parent_id].first_markupmirror = content
